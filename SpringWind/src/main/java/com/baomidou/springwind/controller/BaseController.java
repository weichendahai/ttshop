...
public class BaseController extends SuperController implements HandlerInterceptor {
	...
	protected  String multiToJson(Integer size,List<?> list){
		JSONObject jo = new JSONObject();
		jo.put("total",size);
		jo.put("rows",list);
		return toJson(jo);
	...
	protected Integer getLimitIndex(){
		Integer index = Integer.parseInt(request.getParameter("_index"));
		return 	index!=null?index:null;
	}

	protected Integer getLimitSize(){
		Integer size = Integer.parseInt(request.getParameter("_size"));
